...
| 2026-06-26 | Couple | Gallery / images | **Photos load very slowly** — needs lazy-loading + caching (and likely served thumbnails / responsive sizes instead of full-res for the grid). *Ratified 2026-07-08: grid fires all 86 full-res originals at once; no `loading="lazy"` in `Gallery.tsx`, no thumbnail sizes in the schema (`gallery_items` stores only `file_path`)*. *2026-07-09: fixed on main (frontend), pending deploy: lazy-loading + decoding=async + 24-per-page "Show more"; also fixed nav doing a full page reload on every click (raw `<a>` → SPA links). True thumbnails still need backend work* | 🟡 minor | in-progress |
| 2026-07-08 | IT/Admin | Guest footer | **Placeholder support email** — `support@wedding.example.com` hardcoded in `GuestLayout.tsx:85`, shown on every guest page; the mailto goes nowhere. *2026-07-09: fixed on main (frontend), pending deploy: now Ajandrews210888@aol.com* | 🟡 minor | in-progress |
| 2026-07-08 | IT/Admin | Gallery (data) | **85/86 photo titles are raw filenames** (e.g. "20260613_120148.jpg") — the bulk load set `title` = filename (verified in prod DB). Frontend fallback is "Photo {id}", so fix = null/replace the seeded titles, or friendlier frontend handling. *2026-07-09: fixed on main (frontend), pending deploy: frontend now shows a friendly date for timestamp filenames and hides camera-style names; DB cleanup optional* | 🟡 minor | in-progress |
| 2026-07-08 | IT/Admin | Schedule | **Ceremony never appears on the Schedule page** — it lists only `events` rows (2 exist); the ceremony lives on `weddings.ceremony_time` so guests see rehearsal dinner + wedding breakfast but not the ceremony itself. Also data checks for the couple: Wedding Breakfast is 11:00, *before* the 12:00 ceremony; Rehearsal Dinner has no time set; guest page shows no year on dates. *2026-07-09: fixed on main (frontend), pending deploy: Schedule now synthesizes the ceremony from the wedding record (skipped if a ceremony event exists) and dates include the year. Data checks (11:00 breakfast, missing rehearsal time) still with the couple* | 🟡 minor | in-progress |

## 💡 Ideas / feature requests
| Date | Source | Idea | Why it helps | Status |
|------|--------|------|--------------|--------|
| 2026-06-26 | Couple | Gallery: **slideshow view** of approved photos | A nicer way to view the album / show it off on the day. *2026-07-09: fixed on main (frontend), pending deploy: play/pause auto-advance in the lightbox* | in-progress |
| 2026-06-26 | Couple | Gallery: **click a photo to open it full-size in a modal/lightbox** (grid currently shows small thumbnails only) | View photos properly instead of tiny tiles; pairs with the slideshow. *2026-07-09: fixed on main (frontend), pending deploy: lightbox with prev/next, arrow keys, Escape* | in-progress |
...
| 2026-07-08 | IT/Admin | **Per-route page titles** — every guest page shows "Wedding Dashboard" in the browser tab (static `<title>` in `index.html`) | Readable tabs, history, bookmarks. *2026-07-09: fixed on main (frontend), pending deploy* | in-progress |
...
| 2026-07-08 | IT/Admin | **Gallery a11y cleanup** — each photo caption is a heading (86 headings on one page) and alt text just repeats the title; use figure/figcaption + meaningful alt | Screen-reader navigability. *2026-07-09: fixed on main (frontend), pending deploy: figure/figcaption, meaningful alt, no heading-spam* | in-progress |
| 2026-07-08 | IT/Admin | **Upload form should state accepted types/size limits** (images only today; guests already tried videos) | Avoids silent failures at the point of upload. *2026-07-09: fixed on main (frontend), pending deploy: "Photos only, up to 25 MB" hint under the file input* | in-progress |
