...
test('creates, edits, and deletes a task', async ({ page }) => {
  ...
  await expect(page.getByRole('status')).toHaveText('Task added successfully.')
  ...
  // Edit: move to Done
  await page.getByRole('button', { name: 'Edit Order flowers' }).click()
  ...
  await expect(page.getByRole('status')).toHaveText('Task updated successfully.')
  const done = mainRegion(page).getByLabel('Done column')
  await expect(done.getByText('Order flowers')).toBeVisible()

  // Delete
  await page.getByRole('button', { name: 'Delete Order flowers' }).click()
  ...
  await expect(page.getByRole('status')).toHaveText('Task deleted successfully.')
